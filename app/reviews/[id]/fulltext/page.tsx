    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-56 shrink-0 border-r border-gray-200 bg-white flex flex-col overflow-hidden">
        <div className="px-3 py-2 border-b border-gray-200 shrink-0">
          <p className="text-xs font-semibold text-gray-700">Full Text Screening</p>
          ...
          {allArticles.map((article) => (
            ...
            >
              <div className="flex items-start gap-1.5">
                <div className={`w-2 h-2 rounded-full mt-1 shrink-0 ${article.status === "included" ? "bg-green-500" : article.status === "excluded" ? "bg-red-500" : "bg-gray-300"}`} />
                <div>
                  <p className="text-xs font-medium text-gray-800 leading-snug line-clamp-2">{article.title}</p>
                  <p className="text-xs text-blue-500 mt-0.5 truncate">{article.authors.split(",")[0]}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {selected ? (
          <>
            <div className="flex-1 overflow-y-auto p-4">
              <h2 className="text-sm font-bold text-gray-800 mb-3 leading-snug">{selected.title}</h2>
              <p className="text-xs text-gray-500 mb-1">{selected.authors}</p>
              <p className="text-xs text-gray-500 mb-4">{selected.journal} • {selected.year}</p>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm font-semibold text-gray-700">Full text screening complete!</p>
            </div>
          </div>
        )}
      </div>
